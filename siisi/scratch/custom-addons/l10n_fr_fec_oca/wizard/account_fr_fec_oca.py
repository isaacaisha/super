...
class AccountFrFecOca(models.TransientModel):
    ...
    def _default_partner_account_ids(self):
        ...
        #)
        #return pay + rec
        # Try to obtain the model safely
        IrProperty = self.env.get("ir.property")
        if not IrProperty:
            # If not found, return an empty recordset on account.account
            return self.env["account.account"]
        # Use sudo() to avoid potential access restrictions
        IrProperty = IrProperty.sudo()
        pay = IrProperty._get("property_account_payable_id", "res.partner")
        rec = IrProperty._get("property_account_receivable_id", "res.partner")
        return pay + rec
